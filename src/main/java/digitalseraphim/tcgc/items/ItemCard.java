...
public class ItemCard extends ItemMap {
	...
	public boolean onDroppedByPlayer(ItemStack item, EntityPlayer player) {
		if (ItemCard.getCardCount(item) == 1) {
			...
			player.dropPlayerItemWithRandomChoice(createItemStack(TCGCraft.proxy.cardItem, new CardInstance[] { drop }), false);
	...
	public static ItemStack createItemStack(Item item, CardInstance[] cards) {
		...
		for (int i = 0; i < cards.length; i++) {
			tagCompound.setTag(NBT_CARD_BASE + i, cards[i].toTagCompound());
		}
