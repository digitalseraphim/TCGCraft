...
public abstract class Card extends WeightedRandomChestContent {
	...
	public Card(String name, Type t, int earthCost, int fireCost, int airCost, int waterCost, int orderCost,
		...
		super(TCGCraft.proxy.cardItem, 0, 2, 3, (rarity.ordinal() + 1) * 5);
		this.name = name;
		this.type = t;
		this.rarity = rarity;
		...
		public void cast(EntityPlayer player, float x, float y, float z) {
			...
			for (int i = -5; i <= 5; i++) {
				for (int j = -5; j <= 5; j++) {
					...
					Block b = player.worldObj.getBlock((int) px + i, (int) py, (int) pz + j);

					if (b == null || b.isReplaceable(player.worldObj, px + i, py, pz + j)) {
						player.worldObj.setBlock(px + i, py, pz + j, ModBlocks.magicFlowingWater, md, 7);
					}
				}
			}
		}
